...
<body>
  ...
  <div class="row">
    ...
    <label>or Upload ZIP: <input id="repoZip" type="file" accept=".zip" /></label>
...
(function(){
  ...
  function startStream(body){
    showSpinner(true);
    var isForm = (body && body instanceof FormData);
    var init = isForm
      ? { method: 'POST', body: body }
      : { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body || {}) };
    fetch('/api/analyze', init).then(function(res){
      if (!res.ok) {
  ...
  runBtn.addEventListener('click', function(e){
    ...
    if (fileInput) {
      var fd = new FormData();
      fd.append('repo', currentRepo);
      fd.append('zip', fileInput, fileInput.name);
